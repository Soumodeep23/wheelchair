...
import pyttsx3
import os # Custom module for window resolution
...
class CameraGridUI:
    ...
    def on_click(self, event):
        ...
        # Compute mapping ratios
        ratio_x = cam_width / w_screen
        ratio_y = cam_height / h_screen

        # Compute grid dimensions on screen
        col_width = w_screen / self.grid_cols
        row_height = h_screen / self.grid_rows

        # Identify clicked grid
        col = int(event.x // col_width)
        row = int(event.y // row_height)

        # Map to actual camera coordinate
        x_cam = event.x * ratio_x
        y_cam = event.y * ratio_y

        print(f"Clicked grid: Row {row+1}, Col {col+1}")
        print(f"Screen click: ({event.x:.1f}, {event.y:.1f}) → Camera coords: ({x_cam:.1f}, {y_cam:.1f})")

        # Now send to the arm or further processing
